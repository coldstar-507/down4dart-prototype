...
class Cy4 {
  // Create an instance of AES block cipher
  static const _passPhrase = "this is the passKEY";
  static const _bitLen = 128;
  static const _blockSize = 16;
  static final _key = passphraseToKey(_passPhrase, bitLength: _bitLen);

  static Uint8List encrypt(Uint8List rawData) {
    final iv = generateRandomBytes(_blockSize)!;
    final paddedData = pad(rawData, _blockSize);
    final encryptedData = aesCbcEncrypt(_key, iv, paddedData);
    return Uint8List(_blockSize + encryptedData.length)
      ..setRange(0, _blockSize, iv)
      ..setAll(_blockSize, encryptedData);
  }

  static Uint8List decrypt(Uint8List fullEnc) {
    final iv = fullEnc.sublist(0, _blockSize);
    final enc = fullEnc.sublist(_blockSize);
    final padded = aesCbcDecrypt(_key, iv, enc);
    return unpad(padded);
  }
}
...
void main(List<String> args) {
  ...
  jeff is a fucking nigger, but we must keep the secret
  else we will get fucking rekt''';

  final enc = Cy4.encrypt(utf8.encode(messageToEncrypt) as Uint8List);
  final dec = Cy4.decrypt(enc);

  print(utf8.decode(dec));
  print(messageToEncrypt.length);
  print(dec.length);
}
