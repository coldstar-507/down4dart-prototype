...
class Down4TX {
  ...
  List<int> hashBuf;
  bool settled;
  List<List<int>> offLineDependance;
  Down4TX({
    required this.utxos,
    required this.stxis,
...
class Wallet {
  ...
  Tuple4<bsv.Tx, List<Down4STXI>, bsv.BigIntX, List<List<int>>>? _inputReadyTx(
    final bsv.BigIntX pay,
    Node self,
  ) {
    var txb = bsv.TxBuilder();
    ...
    for (var i = 0; i < _utxos.length; i++) {
      ...
      if (sats.geq(requiredSats)) {
        ...
        return Tuple4(txb.tx, d4txins, extraSats, unsettledTxDepedance);
      }
    }
    return null;
  }
